...
class Pengguna extends CI_Controller {
	...
	$now = new DateTime();
        ...
        if(!$getAccess && $setLogin["name"] == $neMSC){
            ...
            $bUrl = base_url();
            $message = $this->sendmail->secureMail($data['first_name'],$data['last_name'],$data['username'],$dTod,$dTim,$stLe,$browser,$os,$getip,$bUrl);
            $to_email = $data['email'];
	...
	public function settings(){
	    ...
	    if($dataLevel == "is_admin"){
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(site_url().'pengguna/settings/');
            }
	    }
	...
	public function changelevel()
	{
	    ...
	    if($dataLevel == "is_admin"){
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(site_url().'pengguna/changelevel');
            }
	    }else{
	        redirect(site_url().'pengguna/');
	    }
	...
	public function banuser() 
	{
	    ...
	    if($dataLevel == "is_admin"){
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                redirect(site_url().'pengguna/banuser');
            }
	    }else{
	        redirect(site_url().'pengguna/');
	    }
	    ...
	    if($dataLevel == "is_admin"){
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if($this->user_model->isDuplicate($this->input->post('username'))){
                    $this->session->set_flashdata('flash_message', 'User username already exists');
                    redirect(site_url().'pengguna/adduser');
                }else{
                    $this->load->library('password');
                    $post = $this->input->post(NULL, TRUE);
                    $cleanPost = $this->security->xss_clean($post);
    ...
    public function register()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->user_model->isDuplicate($this->input->post('username'))){
                $this->session->set_flashdata('flash_message', 'User username already exists');
                redirect(site_url().'pengguna/register');
            }else{
                $post = $this->input->post(NULL, TRUE);
                ...
                if($data['recaptcha'] == 'yes'){
                    ...
                    if($status['success']){
                        ...
                        $url = site_url() . 'pengguna/complete/token/' . $qstring;
                        $link = '<a href="' . $url . '">' . $url . '</a>';
    
                        $this->load->library('username');
                        $this->load->library('sendmail');
                        
                        $message = $this->sendmail->sendRegister($this->input->post('lastname'),$this->input->post('username'),$link, $sTl);
                        $to_username = $this->input->post('username');
                        $this->username->from($this->config->item('register'), 'Set Password ' . $this->input->post('firstname') .' '. $this->input->post('lastname')); //from sender, title username
                        $this->username->to($to_username);
                        $this->username->subject('Set Password Login');
                        $this->username->message($message);
                        $this->username->set_mailtype("html");
    
                        //Sending mail
                        if($this->username->send()){
                            redirect(site_url().'pengguna/successregister/');
                        }else{
                            $this->session->set_flashdata('flash_message', 'There was a problem sending an username.');
                            exit;
                        }
                    }else{
                        //recaptcha failed
        ...
        if(!empty($data['username'])){
            ...
            if($this->form_validation->run() == FALSE) {
                ...
                if($data['recaptcha'] == 'yes'){
                    ...
                    if(!$userInfo)
                    {
                        $this->session->set_flashdata('flash_message', 'Wrong password or username.');
                        redirect(site_url().'pengguna/login');
                    }
                    ...
                    elseif($status['success'] && $userInfo && $userInfo->banned_users == "unban") //recaptcha check, success login, ban or unban
                    {
                        ...
                        redirect(site_url().'pengguna/checkLoginUser/');
                    }
                    else
                    {
    ...
    public function forgot()
    {
        ...
        if($this->form_validation->run() == FALSE) {
            ...
            $userInfo = $this->user_model->getUserInfoByusername($clean);

            if(!$userInfo){
                $this->session->set_flashdata('flash_message', 'We cant find your username address');
                redirect(site_url().'pengguna/login');
            }
            ...
            if($data['recaptcha'] == 'yes'){
                ...
                if($status['success']){
                    ...
                    $qstring = $this->base64url_encode($token);
                    $url = site_url() . 'pengguna/reset_password/token/' . $qstring;
                    $link = '<a href="' . $url . '">' . $url . '</a>';
    
                    $this->load->library('username');
                    $this->load->library('sendmail');
                    
                    $message = $this->sendmail->sendForgot($this->input->post('lastname'),$this->input->post('username'),$link,$sTl);
                    $to_username = $this->input->post('username');
                    $this->username->from($this->config->item('forgot'), 'Reset Password! ' . $this->input->post('firstname') .' '. $this->input->post('lastname')); //from sender, title username
                    $this->username->to($to_username);
                    ...
                    if($this->username->send()){
                        redirect(site_url().'pengguna/successresetpassword/');
                    }else{
                        $this->session->set_flashdata('flash_message', 'There was a problem sending an username.');
                        exit;
                    }
